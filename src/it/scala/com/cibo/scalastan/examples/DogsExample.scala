...
object DogsExample extends App with ScalaStan {
  ...
  val results = model
    .transform(com.cibo.scalastan.transform.SplitExpressions())
    .transform(com.cibo.scalastan.transform.CSE())
    .transform(com.cibo.scalastan.transform.CopyPropagation())
    .withData(rData(y, "y"))
    .run()
